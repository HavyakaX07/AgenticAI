...
graph TB
    ...
    Orchestrator -->|2. Reason| LLM[Ollama LLM - host]
    Orchestrator -->|2.5 Resolve| Resolver[Device Resolver]
    ...
    APITools -->|Allowlisted| ExtAPI[NMS REST API]

    Resolver -->|brand/IP/type lookup| DeviceDB[(NMS Device DB / Mock)]
    ...
    style Resolver fill:#9B59B6
    style DeviceDB fill:#95A5A6
```
...
sequenceDiagram
    ...
    O->>E: POST /embed + /search
    E-->>O: {capability candidates}

    O->>L: Chat completion (tools + query)
    L-->>O: {decision, tool_name, payload}
    Note over L,O: payload may contain brand names,<br/>IPs, or device types as references

    O->>R: resolve_device_references(payload)
    Note over R: Checks source, destinations,<br/>deviceId, deviceIds fields
    alt Single match
        R-->>O: resolved device_id (pass through)
    else Multiple matches (ambiguous)
        R-->>O: candidates list
        O-->>U: ASK_USER - pick exact device_id
    else No match
        R-->>O: unresolved
        O-->>U: ASK_USER - device not found
    end

    O->>V: Validate resolved payload
    V-->>O: Valid ✓

    O->>P: POST /v1/data/policy
    P-->>O: {allow: true, reason}

    O->>A: POST /tools/invoke {tool, args with real device_ids}
    A-->>O: {status, result}
    O-->>U: {decision: USE_TOOL, result, resolved_devices}
```
